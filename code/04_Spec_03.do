/*******************************************************************************
	...
	Program:  	04_Spec_3_synth.do
	Authors:  	Alex Nowrasteh
				AC Forrester
	
	Purpose: 	3rd Specification
	
	- outcome: N cases
	- treatment date: 2/2
	- time: days since 1st case
...
*******************************************************************************/
	...
	* response variable
	local response confirmed
	
	* control variables
	local controls un_migrant pop_total pop_density pct_oldpop pct_urban un_medage gdppc2011 abslat n_migrant_china n_connects
			
	* initialize temp file
	tempfile tmp
	
	* load dataset	
	use "${output}synth_data_since_case", clear
			
	* find SC unit
	synth `response' `controls',	trunit(`treated')        	///
									trperiod(`tr_preiod')    	///
									keep(${output}spec_03_plot)	///
									replace                 	///
									nested allopt 
	...
	* save output results
	save ${output}synth_spec_03, replace
